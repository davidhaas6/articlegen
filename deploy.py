...
def generate_and_push_articles(repo_url: str, num_articles: int = 5) -> str:
    ...
    git_deploy(auth_repo_url(repo_url), site_dir)
...
def git_deploy(repo_url: str, site_dir: str, branch_name: str = 'main') -> None:
...
def run_command(command):
    try:
        output = subprocess.run(command, check=True, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True)
...
if __name__ == "__main__":
    ...
    generate_and_push_articles(default_repo, 4)
...
#         subprocess.run(
#             ["git", "ls-remote", "--exit-code", "--heads", "origin", "main"], capture_output=True
#         ).returncode
#         == 0
#     )
...
# def replace_repo_files(repo_url: str, new_files_dir: str, github_token: str = None, push: bool = False):
#     def run_command(cmd):
#         try:
#             output = subprocess.run(cmd, check=True, capture_output=True, text=True)
#             logging.info(f'{cmd} -> {output.stdout.strip()}')
#             return True
#         except subprocess.CalledProcessError as e:
#             logging.error(f"Command failed: {e.cmd}. Error: {e.stderr.strip()}")
#             return False

#     try:
#         new_files_dir = os.path.abspath(new_files_dir)

#         parsed_url = urlparse(repo_url)
#         username, repo_name = parsed_url.path.strip('/').split('/')[:2]
#         repo_name = repo_name.rstrip('.git')

#         clone_url = f"https://{github_token}@github.com/{username}/{repo_name}.git" if github_token else repo_url
#         print(clone_url)
        
#         os.chdir(new_files_dir)
#         if not run_command(['git', 'clone', clone_url]):
#             return
...
#         for item in os.listdir('.'):
#             if item != '.git':
#                 (os.remove if os.path.isfile(item) else shutil.rmtree)(item)  # concise way to conditionally execute one of two functions

#         for item in os.listdir(new_files_dir):
#             if repo_name in item:
#                 continue
#             s, d = os.path.join(new_files_dir, item), os.path.join('.', item)
#             (shutil.copy2 if os.path.isfile(s) else shutil.copytree)(s, d)

#         if not all([
#             run_command(['git', 'add', '-A']),
#             run_command(['git', 'commit', '-m', "Replace all files with new content"]),
#             not push or run_command(['git', 'push', f"https://{github_token}@github.com/{username}/{repo_name}.git" if github_token else 'origin'])
#         ]):
#             return
